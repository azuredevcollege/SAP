...
|scope|https://SAML_AAD_CONFIG2/user_impersonation|A space-separated list of scopes. As set up in the AAD previously.|
...
|assertion|jwt token|The value of the token used in the request. This token must have an audience of the app making this OBO request (the app denoted by the client-id field). In this case the access token we got from the Implicit Grant Flow.|
...
|resource|https://SAML_AAD_CONFIG2|A space separated list of scopes for the token request. As set up within AAD and SAP.|
